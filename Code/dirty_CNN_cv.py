# -*- coding: utf-8 -*-
"""
Created on Tue Nov 28 14:29:53 2017
...
train_file = "../Dataset/dirty.pkl.gz"
k_fold = 10
...
W5 = tf.Variable(tf.truncated_normal([H, 3], stddev=0.1), name="W5")
B5 = tf.Variable(tf.constant(0.1, tf.float32, [3]), name="B5")
...
test_batch_X, test_batch_Y = testset.dirty_minibatch(len(testY))
test_pred = []
...
for k in range(k_fold):
    ...
    for i in range(iteration):
        ...
        batch_x, batch_y = trainset.dirty_minibatch(train_size)
        ...
        if (i % test_frequency == 0):
            test_batch_x, test_batch_y = validset.dirty_minibatch(test_size)
    ...
    model_path = "dirty_cv_model_"+str(k)
...
for i in range(prediction.shape[0]):
    stat[test_batch_Y.argmax(axis=1)[i]][prediction[i]] += 1

f = open('plot_dirty_cv', 'wb')
